...
class Recipe(models.Model):
    ...
    comment = models.ForeignKey(Comment, related_name='recipes',
                                on_delete=models.CASCADE,
                                null=True, blank=True)
...
class Submission(models.Model):
    ...
    comment = models.ForeignKey(Comment, related_name='submissions',
                                on_delete=models.CASCADE,
                                null=True, blank=True)
    featured_image = CloudinaryField('image', default='placeholder')
    username_id = models.ForeignKey(User, on_delete=models.CASCADE,
                                    default=1, related_name="submitter",
                                    unique=False)
    created_on = models.DateTimeField(auto_now_add=True)
    likes = models.ManyToManyField(User, related_name='submission_likes',
                                   blank=True)
    status = models.IntegerField(choices=STATUS, default=0)
